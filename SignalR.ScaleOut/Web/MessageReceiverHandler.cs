...
namespace SignalR.Web
{
    public class MessageReceiverHandler : IHttpHandler
    {
        ...
        {
            var signalBus = Signaler.Instance.SignalBus as PeerToPeerSignalBusMessageStore;
            if (signalBus == null)
            {
                return;
            }
            ...
            var generateMessageId = context.Request.QueryString[PeerToPeerHelper.RequestKeys.GenerateMessageId];
